...
export default function CompletedGamesYearsPage() {
  ...
  useEffect(() => {
    api
      .listCompletedYears()
      .then(setYears)
      .catch((err) => setError(err instanceof Error ? err.message : "Nie udało się pobrać dostępnych lat."))
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <Link href="/completed-games" className="inline-flex min-h-11 items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
            <ArrowLeft className="h-4 w-4" aria-hidden="true" /> Bieżący rok
          </Link>
          <p className="mt-2 text-sm font-semibold text-primary">Historia</p>
          <h1 className="mt-1 text-2xl font-bold leading-tight sm:text-3xl">Inne lata</h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
            Wybierz rok, aby zobaczyć ukończone gry i jego podsumowanie.
          </p>
        </div>
      ...
      {!loading && !error && !years.length ? (
        <EmptyState title="Brak wcześniejszych ukończonych gier" description="Dodaj pierwszy wpis, aby rozpocząć historię." />
      ) : null}
      {!loading && !error && years.length ? (
