  ...
  return (
    <Card className="min-w-0">
      <CardHeader>
        ...
        <CardDescription>Wybierz dwa lata, aby zestawić liczbę gier, ligi PoE, łączny czas, oceny i miesięczny trend ukończeń.</CardDescription>
      </CardHeader>
      <CardContent className="min-w-0 space-y-5">
        <div className="grid max-w-2xl gap-3 sm:grid-cols-2">
          <div className="space-y-1.5"><Label htmlFor="comparison-year-a">Pierwszy rok</Label><Select id="comparison-year-a" value={firstYear ?? ""} onChange={(event) => secondYear !== undefined && onSelectedYearsChange([Number(event.target.value), secondYear])}>{availableYears.map((item) => <option key={item.year} value={item.year} disabled={item.year === secondYear}>{yearOptionLabel(item)}</option>)}</Select></div>
          <div className="space-y-1.5"><Label htmlFor="comparison-year-b">Drugi rok</Label><Select id="comparison-year-b" value={secondYear ?? ""} onChange={(event) => firstYear !== undefined && onSelectedYearsChange([firstYear, Number(event.target.value)])}>{availableYears.map((item) => <option key={item.year} value={item.year} disabled={item.year === firstYear}>{yearOptionLabel(item)}</option>)}</Select></div>
        </div>
...
function ComparisonData({ comparison }: { comparison: CompletedGamesComparison }) {
  ...
  return <div className="min-w-0 space-y-5">
    <div className="grid gap-3 sm:grid-cols-2">
      {comparison.years.map((year, index) => <div key={year.year} className="rounded-lg border border-border bg-background/55 p-4"><p className="text-lg font-bold">{year.year}</p><dl className="mt-3 grid gap-2 text-sm"><Metric icon={BarChart3} label="Ukończone gry" value={String(year.completed_games_count)} /><Metric icon={BarChart3} label="Ligi PoE" value={String(year.poe_leagues_count ?? 0)} /><Metric icon={Clock3} label="Łączny czas" value={(year.combined_playtime_hours ?? year.total_playtime_hours) > 0 ? formatHours(year.combined_playtime_hours ?? year.total_playtime_hours) : "Brak danych"} /><Metric icon={Clock3} label="W tym PoE" value={(year.poe_playtime_hours ?? 0) > 0 ? formatHours(year.poe_playtime_hours ?? 0) : "Brak danych"} /><Metric icon={Clock3} label="Średni czas gry" value={year.average_playtime_hours == null ? "Brak danych" : formatHours(year.average_playtime_hours)} /><Metric icon={Star} label="Średnia ocena" value={year.average_rating == null ? "Brak ocen" : `${formatNumber(year.average_rating)}/10`} /></dl><span className={`mt-3 block h-1.5 w-12 rounded ${comparisonColors[index]}`} aria-hidden="true" /></div>)}
    </div>
...
function yearOptionLabel(item: CompletedGamesYear) { return `${item.year} (${item.completed_games_count} gier${item.poe_leagues_count ? ` · ${item.poe_leagues_count} lig PoE` : ""})`; }
